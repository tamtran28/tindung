...
def process_crm_data(
    ...
    cols_order = [col for col in cols_order if col in pivot_final.columns]
    ...
    # Top 10 KHCN/KHDN
    # Đảm bảo cột 'DƯ NỢ' tồn tại và là số
    pivot_full['DƯ NỢ'] = pd.to_numeric(pivot_full['DƯ NỢ'], errors='coerce').fillna(0)
    
    top10_khcn_cif = pivot_full[pivot_full['CUSTTPCD'] == 'Ca nhan'].nlargest(10, 'DƯ NỢ')['CIF_KH_VAY']
    pivot_full['Top 10 dư nợ KHCN'] = pivot_full['CIF_KH_VAY'].apply(lambda x: 'x' if x in top10_khcn_cif.values else '') # Sửa Top 5 thành Top 10

    top10_khdn_cif = pivot_full[pivot_full['CUSTTPCD'] == 'Doanh nghiep'].nlargest(10, 'DƯ NỢ')['CIF_KH_VAY']
    pivot_full['Top 10 dư nợ KHDN'] = pivot_full['CIF_KH_VAY'].apply(lambda x: 'x' if x in top10_khdn_cif.values else '') # Sửa Top 5 thành Top 10


    # TSBĐ quá hạn định giá
    ngay_danh_gia_tsbd = pd.to_datetime("2025-03-31") # Cần đảm bảo ngày này hợp lý
    loai_ts_r34 = ['BĐS', 'MMTB', 'PTVT']
    df_crm4_for_tsbd = df_crm4_filtered.copy() # Làm việc trên bản sao
    ...
    # df_crm4_for_tsbd.loc[mask_r34, 'SO_THANG_QUA_HAN'] = ( # Cột này không được sử dụng tiếp
    #     ((ngay_danh_gia_tsbd - df_crm4_for_tsbd.loc[mask_r34, 'VALUATION_DATE']).dt.days / 31) - 12
    # )
    cif_quahan = df_crm4_for_tsbd[
    ...
    if df_sol_data is not None and dia_ban_kt_filter:
        ...
        df_bds_matched['CANH_BAO_TS_KHAC_DIABAN'] = df_bds_matched['TINH_TP_TSBD'].apply(
            lambda x: 'x' if x and x != dia_ban_kt_filter else ''
        )
        ma_ts_canh_bao = df_bds_matched[df_bds_matched['CANH_BAO_TS_KHAC_DIABAN'] == 'x']['C01'].unique()
        # Map mã TS về CIF từ df_crm4_filtered (đã được lọc theo chi nhánh)
        ...
        st.warning("Không có dữ liệu Mục 17 (df_sol) hoặc địa bàn kiểm toán để xử lý TSBĐ khác địa bàn.")
        df_bds_matched = pd.DataFrame() # Trả về df rỗng nếu không xử lý


    # Giao dịch giải ngân/tất toán trong ngày
    df_gop = pd.DataFrame() # Khởi tạo để tránh lỗi nếu không có file
    ...
    if df_55_data is not None and df_56_data is not None:
        ...
        # ds_nhieu_tt_gn = df_count[ # Cột này không được sử dụng trong pivot_full cuối
        #     (df_count['Giải ngân'] > 1) | (df_count['Tất toán'] > 1)
        # ]['CIF'].unique()
    ...
    # Xử lý chậm trả
    df_delay_processed = pd.DataFrame() # Khởi tạo
    if df_delay_data is not None:
        ...
        ngay_danh_gia_cham_tra = pd.to_datetime("2025-03-31") # Cần đảm bảo ngày này hợp lý
        ...
        mask_period = df_delay['NGAY_DEN_HAN_TT'].dt.year.between(2023, 2025) # Bao gồm cả 2025
        df_delay = df_delay[mask_period]

        # Đổi tên cột trong pivot_full tạm thời để merge
        ...
        # Chỉ lấy nợ nhóm 1 (sau khi đã merge NHOM_NO từ pivot_full)
        df_delay = df_delay[df_delay['NHOM_NO'] == 1.0] # So sánh với float nếu NHOM_NO là float
        ...
        df_delay['NGAY_DEN_HAN_TT_DATE'] = df_delay['NGAY_DEN_HAN_TT'].dt.date # Sử dụng cột mới cho sort và drop_duplicates
        ...
        df_delay_processed = df_delay # Gán lại df_delay đã xử lý
    else:
...
with st.sidebar:
    ...
    uploaded_giai_ngan_file = st.file_uploader("6. File Giai_ngan_tien_mat_1_ty.xls", type=["xls","xlsx"], key="giai_ngan")
...
if st.button("🚀 Bắt đầu xử lý dữ liệu", key="process_button"):
    ...
    if not all([uploaded_crm4_files, uploaded_crm32_files, uploaded_muc_dich_file, uploaded_code_tsbd_file, uploaded_sol_file, uploaded_giai_ngan_file, uploaded_55_file, uploaded_56_file, uploaded_delay_file]):
        st.error("Vui lòng tải lên đầy đủ tất cả 9 loại file được yêu cầu ở thanh bên.")
    elif not chi_nhanh_input:
        st.error("Vui lòng nhập tên chi nhánh hoặc mã SOL.")
    # Địa bàn kiểm toán có thể không bắt buộc với mọi logic, nhưng nếu cần thì thêm check
    # elif not dia_ban_kt_input:
    # st.error("Vui lòng nhập tỉnh/thành kiểm toán.")
    else:
        with st.spinner("⏳ Đang tải và xử lý dữ liệu... Vui lòng chờ."):
            ...
            if df_crm4_raw is None or df_crm32_raw is None or \
               ...
               df_sol_data is None or df_giai_ngan_data is None or \
               df_55_data is None or df_56_data is None or df_delay_data is None:
                st.error("Một hoặc nhiều file không thể đọc được. Vui lòng kiểm tra lại file đã tải lên.")
            else:
                try:
                    ...
                    if results:
                        ...
                        with pd.ExcelWriter(output, engine='openpyxl') as writer:
                            if df_crm4_filtered_res is not None: df_crm4_filtered_res.to_excel(writer, sheet_name='df_crm4_LOAI_TS', index=False)
                            if pivot_final_res is not None: pivot_final_res.to_excel(writer, sheet_name='KQ_CRM4', index=False)
                            if pivot_merge_res is not None: pivot_merge_res.to_excel(writer, sheet_name='Pivot_crm4', index=False)
                            if df_crm32_filtered_res is not None: df_crm32_filtered_res.to_excel(writer, sheet_name='df_crm32_MUC_DICH', index=False) # Đổi tên sheet cho phù hợp
                            if pivot_full_res is not None: pivot_full_res.to_excel(writer, sheet_name='KQ_KH', index=False)
                            if pivot_mucdich_res is not None: pivot_mucdich_res.to_excel(writer, sheet_name='Pivot_crm32', index=False)
                        ...
                        st.download_button(
                            ...
                            data=excel_data,
                            file_name="KQ_XuLy_CRM.xlsx", # Đổi tên file kết quả
